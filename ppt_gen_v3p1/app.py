...
def build_slides(token: str,
                 engine: str = Query(
                     "auto",
                     pattern="^(auto|library|libreoffice|powerpoint|graph|svg)$")):
    ...
    Four renderers, and the answer says which one drew it:
      ...
      **graph** — Microsoft Graph converts the built deck with Office Online's
      own renderer and the PDF is rasterised here. Values show and the fidelity
      is Office's, with nothing installed anywhere — at the price of a network
      round trip and the deck passing briefly through SharePoint. Under
      evaluation: it is never chosen by `auto`, only by name.
