"""Read-only OOXML parsing: what is on a slide, and is it the same slide?

Vendored from v1's `pptx_forensics.py`, for the reason the validator was: on a
standalone deployment the parent folder holds no v1, the optional import found
nothing, and slide identity silently degraded to hashing whole slides — which
matches nothing across decks, so a merge de-duplicated nothing at all. An
import that can fail is not a foundation to build identity on.

The important idea, and it is v1's: when Templafy assembles a deck it COPIES
slides out of the big master template. Copied shapes keep their Microsoft
creationId GUIDs (`a16:creationId`), and those survive text edits — injecting a
client name does not change them. That is what makes it possible to say two
decks contain the same slide when their text differs.

Keep in step with v1's copy if that one changes.
"""
