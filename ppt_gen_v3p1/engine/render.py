"""Render a deck to slide images. Five backends, one interface.
...
So we hand the file to something that renders it properly:
  ...
  **graph** — Microsoft Graph converts the built deck to PDF with Office
  Online's own renderer, and it is rasterised here. Values show, fidelity is
  Office's, and nothing has to be installed on the machine doing it. The price
  is a network round trip, an Entra ID app registration, and the deck passing
  briefly through SharePoint — `engine/graph.py` explains why that last one is
  unavoidable. Being evaluated; last in ORDER, so nothing reaches it by
  accident.
...
Configuration, all optional:
    ...
    GRAPH_*                 the graph backend's four settings, in
                            `engine/graph.py`. Never in code, never committed.
...
# Preference order; svg is the caller's job. `graph` is last deliberately:
# it is the one under evaluation, and it is the only one that leaves the
# machine, so nothing should reach it without asking for it by name.
ORDER = ("libreoffice", "powerpoint", "graph")
...
# ---------------------------------------------------------------- Graph
#
# Office Online's converter, over the network. It renders the *built* deck, so
# filled values show — the one thing the library backend cannot do — and it
# needs no software installed anywhere, which is what makes it interesting for
# a deployment where LibreOffice is not allowed and PowerPoint cannot run.
#
# What it costs is in `engine/graph.py`: Graph converts a file that is already
# in a drive, never a stream of bytes, so the deck goes to SharePoint and comes
# back out again. The delete is not best effort; see `to_pdf`.
def _probe_graph():
    """Cheap and offline, like every probe here. This is called on ordinary
    page loads, and a probe that fetched a token would put a round trip and a
    possible outage on each one."""
    gone = graph.missing_config()
    if gone:
        return False, ("Graph is not configured: %s %s not set"
                       % (", ".join(gone), "is" if len(gone) == 1 else "are"))
    try:
        import msal                                              # noqa: F401
    except ImportError:
        return False, "msal is not installed (pip install msal)"
    try:
        import pypdfium2                                         # noqa: F401
    except ImportError:
...
def _export_graph(pptx, out_dir, width):
    work = tempfile.mkdtemp(prefix="pptgen-graph-")
    try:
        pdf = os.path.join(work, "deck.pdf")
        with open(pdf, "wb") as fh:
            fh.write(graph.from_env(timeout=TIMEOUT).to_pdf(pptx))
        return _rasterise(pdf, out_dir, width)
    except graph.GraphError as exc:
        # Already redacted and already phrased for a human; anything else would
        # only add a traceback nobody on this screen can act on.
        raise RenderError("Graph: %s" % exc)
    except Exception as exc:
        raise RenderError("%s: %s" % (type(exc).__name__, exc))
    finally:
        shutil.rmtree(work, ignore_errors=True)
...
BACKENDS = {"libreoffice": (_probe_libreoffice, _export_libreoffice),
            ...
            "graph": (_probe_graph, _export_graph)}
